...
func (h *AuditHandler) GetAuditLogs(w http.ResponseWriter, r *http.Request) {
	...
	// Get user from context - check role to determine access level
	userRole, err := supabase.GetUserRoleFromContext(r.Context())
	if err != nil {
		// Fallback: try to get user ID from legacy auth
		...
		userIDVal := userID
		targetUserID = &userIDVal
	} else {
		// Admin/super_admin can see all logs, regular users only see their own
		...
		// If admin/super_admin, targetUserID remains nil (see all logs)
	}

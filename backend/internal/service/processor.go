...
const (
	EmployeeImportModeMerge  EmployeeImportMode = "merge"
	EmployeeImportModeInsert EmployeeImportMode = "insert"
	EmployeeImportModeUpdate EmployeeImportMode = "update"
)

func ParseEmployeeImportMode(mode string) EmployeeImportMode {
	normalized := strings.ToLower(strings.TrimSpace(mode))
	switch normalized {
	case string(EmployeeImportModeInsert):
		return EmployeeImportModeInsert
	case string(EmployeeImportModeUpdate):
		return EmployeeImportModeUpdate
	case string(EmployeeImportModeMerge), "", "upsert", "all":
		return EmployeeImportModeMerge
	default:
		return EmployeeImportModeMerge
	}
}

func (m EmployeeImportMode) IsValid() bool {
	switch m {
	case EmployeeImportModeMerge, EmployeeImportModeInsert, EmployeeImportModeUpdate:
		return true
	default:
		return false
	}
...
type EmployeeImportOptions struct {
	Mode               EmployeeImportMode
	DefaultStatus      string
	ForceDefaultStatus bool
}

type EmployeeImportConflict struct {
...
type EmployeeImportConflictError struct {
	Conflicts []EmployeeImportConflict `json:"conflicts"`
}

func (e *EmployeeImportConflictError) Error() string {
	return "employee import conflicts detected"
}
...
var employeeHeaderAliases = map[string]string{
	...
	"手机号":           "phone",
	"手机号码":          "phone",
	"社保编号":          "social_insurance_number",
	"社保个人编号":        "social_insurance_number",
	"个人编号":          "social_insurance_number",
	"公积金编号":         "provident_fund_number",
...
func (p *Processor) ParseEmployeeFile(userID uint, storedPath, originalName string, opts EmployeeImportOptions) (*EmployeeImportResult, error) {
	mode := ParseEmployeeImportMode(string(opts.Mode))
	defaultStatus := normalizeEmployeeStatus(opts.DefaultStatus)
	forceDefault := opts.ForceDefaultStatus
	...
	for _, emp := range existing {
		if normalizeEmployeeStatus(emp.Status) != "active" {
			continue
		}
	...
	now := time.Now()
	skipped := 0
	unique := make(map[string]employeeCandidate)
	var conflicts []EmployeeImportConflict
	...
	for _, row := range rows[1:] {
		...
		setField("social_insurance_number", func(val string) { entry.SocialInsuranceNumber = val })
		setField("provident_fund_number", func(val string) { entry.ProvidentFundNumber = val })
		...
		existingRecord, hasExisting := existingMap[idNumber]
		if hasExisting {
			if defaultStatus == "resigned" && !forceDefault && normalizeEmployeeStatus(existingRecord.Status) != "resigned" {
				conflicts = append(conflicts, EmployeeImportConflict{
					ID:             existingRecord.ID,
					Name:           existingRecord.Name,
					IdentityNumber: existingRecord.IDNumber,
			...
			entry.ID = existingRecord.ID
			if entry.EmployeeID == "" {
				entry.EmployeeID = existingRecord.EmployeeID
			}
			if entry.Department == "" {
				entry.Department = existingRecord.Department
			}
			if entry.Position == "" {
				entry.Position = existingRecord.Position
			}
			if entry.Gender == "" {
				entry.Gender = existingRecord.Gender
			}
			if entry.HireDate == "" {
				entry.HireDate = existingRecord.HireDate
			}
			if entry.Age == "" {
				entry.Age = existingRecord.Age
			}
			if entry.WorkYears == "" {
				entry.WorkYears = existingRecord.WorkYears
			}
			if entry.BirthMonth == "" {
				entry.BirthMonth = existingRecord.BirthMonth
			}
			if entry.Education == "" {
				entry.Education = existingRecord.Education
			}
			if entry.PoliticalStatus == "" {
				entry.PoliticalStatus = existingRecord.PoliticalStatus
			}
			if entry.WorkClothingSize == "" {
				entry.WorkClothingSize = existingRecord.WorkClothingSize
			}
			if entry.SafetyShoeSize == "" {
				entry.SafetyShoeSize = existingRecord.SafetyShoeSize
			}
			if entry.HouseholdType == "" {
				entry.HouseholdType = existingRecord.HouseholdType
			}
			if entry.Ethnicity == "" {
				entry.Ethnicity = existingRecord.Ethnicity
			}
			if entry.NativePlace == "" {
				entry.NativePlace = existingRecord.NativePlace
			}
			if entry.IDAddress == "" {
				entry.IDAddress = existingRecord.IDAddress
			}
			if entry.MaritalStatus == "" {
				entry.MaritalStatus = existingRecord.MaritalStatus
			}
			if entry.SocialInsurance == "" {
				entry.SocialInsurance = existingRecord.SocialInsurance
			}
			if entry.HasBirth == "" {
				entry.HasBirth = existingRecord.HasBirth
			}
			if entry.Phone == "" {
				entry.Phone = existingRecord.Phone
			}
			if entry.SocialInsuranceNumber == "" {
				entry.SocialInsuranceNumber = existingRecord.SocialInsuranceNumber
			}
			if entry.ProvidentFundNumber == "" {
				entry.ProvidentFundNumber = existingRecord.ProvidentFundNumber
			}
			if entry.EmergencyContact == "" {
				entry.EmergencyContact = existingRecord.EmergencyContact
			}
			if entry.EmergencyPhone == "" {
				entry.EmergencyPhone = existingRecord.EmergencyPhone
			}
			if entry.CurrentAddress == "" {
				entry.CurrentAddress = existingRecord.CurrentAddress
			}
			if entry.GraduateSchool == "" {
				entry.GraduateSchool = existingRecord.GraduateSchool
			}
			if entry.Major == "" {
				entry.Major = existingRecord.Major
			}
			if entry.GraduationTime == "" {
				entry.GraduationTime = existingRecord.GraduationTime
			}
			if entry.Email == "" {
				entry.Email = existingRecord.Email
			}
			if entry.Remarks == "" {
				entry.Remarks = existingRecord.Remarks
			}
			if defaultStatus != "" {
				if forceDefault {
					entry.Status = defaultStatus
				} else if entry.Status == "" && existingRecord.Status != "" {
					entry.Status = existingRecord.Status
				}
			} else if entry.Status == "" && existingRecord.Status != "" {
				entry.Status = existingRecord.Status
			}
			if entry.ResignDate == "" && existingRecord.ResignDate != "" {
				entry.ResignDate = existingRecord.ResignDate
			}
			entry.CreatedAt = existingRecord.CreatedAt
		} else {
			if defaultStatus != "" {
				entry.Status = defaultStatus
			}
		...
		unique[idNumber] = employeeCandidate{record: entry, existing: hasExisting}
	}

	if len(unique) == 0 && len(conflicts) == 0 {
		return nil, errors.New("未识别到有效的员工数据，请检查模板格式")
	}

	if defaultStatus == "resigned" && len(conflicts) > 0 && !forceDefault {
		return nil, &EmployeeImportConflictError{Conflicts: conflicts}
	}

	insertedEntries := make([]models.Employee, 0, len(unique))
	updatedEntries := make([]models.Employee, 0, len(unique))
	for _, candidate := range unique {
		if candidate.existing {
			updatedEntries = append(updatedEntries, candidate.record)
		} else {
			insertedEntries = append(insertedEntries, candidate.record)
		}
	}

	insertedCount := len(insertedEntries)
	updatedCount := len(updatedEntries)

	var entriesToPersist []models.Employee
	switch mode {
	case EmployeeImportModeInsert:
		entriesToPersist = make([]models.Employee, len(insertedEntries))
		copy(entriesToPersist, insertedEntries)
		skipped += updatedCount
		updatedCount = 0
	case EmployeeImportModeUpdate:
		entriesToPersist = make([]models.Employee, len(updatedEntries))
		copy(entriesToPersist, updatedEntries)
		skipped += insertedCount
		insertedCount = 0
	default: // merge
		entriesToPersist = make([]models.Employee, 0, len(unique))
		entriesToPersist = append(entriesToPersist, insertedEntries...)
		entriesToPersist = append(entriesToPersist, updatedEntries...)
	}
	...
	updateColumns := []string{
		...
		"marital_status", "social_insurance", "has_birth", "phone", "social_insurance_number", "provident_fund_number", "emergency_contact", "emergency_phone", "current_address",
		"graduate_school", "major", "graduation_time", "email", "remarks", "status", "resign_date", "updated_at",
	}

	if len(entriesToPersist) > 0 {
		if err := p.db.Transaction(func(tx *gorm.DB) error {
			switch mode {
			case EmployeeImportModeInsert:
				if err := tx.Clauses(clause.OnConflict{
					...
					DoNothing: true,
				}).Create(&entriesToPersist).Error; err != nil {
					return fmt.Errorf("insert employees: %w", err)
				}
